...
public class OphthalmoPatternLayer extends GeoRenderLayer<OphthalmoEntity> {

    private static final ResourceLocation MODEL = new ResourceLocation(MarvelousMenagerie.MOD_ID, "geo/ophthalmo.geo.json");

    public OphthalmoPatternLayer(GeoRenderer<OphthalmoEntity> entityRendererIn) {super(entityRendererIn);}
    ...
    public void render(PoseStack poseStack, OphthalmoEntity entityLivingBaseIn, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        ...
        if (entityLivingBaseIn.getPattern()!=0) {
            ...
            RenderType cameo = RenderType.entityCutoutNoCull(
                    new ResourceLocation(MarvelousMenagerie.MOD_ID, "textures/entity/ophthalmo/ophthalmo"+pattern+color+".png"));
